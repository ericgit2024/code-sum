...
def main():
    ...
    parser.add_argument('--max_iterations', type=int, default=None,
                       help='Override max iterations for entity refinement agent')
    ...
    if args.max_iterations is not None:
        ...
        if 'entity_verification' not in config:
            config['entity_verification'] = {}
        config['entity_verification']['max_iterations_eval'] = args.max_iterations
    ...
    # Step 5: Initialize entity refinement agent with eval mode
    print("\n[5/6] Initializing entity refinement agent...")
    entity_agent = EntityRefinementAgent(model, tokenizer, config, eval_mode=True)
    ...
    # Check if entity agent is enabled
    use_entity_agent = (not args.no_entity_agent and 
                     config.get('entity_verification', {}).get('enabled', True))
    
    if not use_entity_agent:
        print("Entity refinement agent disabled - using base model only")
    
    predictions = inference_pipeline.predict_batch(test_data, use_entity_agent=use_entity_agent)
    ...
    results['entity_agent_enabled'] = use_entity_agent
